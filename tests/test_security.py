...
class TestHttpsRedirect:
    """HTTP → HTTPS redirect fires in production; is a no-op in dev/test."""

    def test_http_request_redirects_to_https_in_production(self):
        """X-Forwarded-Proto: http with IS_PRODUCTION=True must return 301 to https."""
        with patch("config.IS_PRODUCTION", True):
            with app.test_client() as c:
                resp = c.get(
                    "/",
        ...
        assert location.startswith("https://"), (
            f"Expected redirect to https://, got: {location}"
        )

    def test_https_request_not_redirected_in_production(self):
        """X-Forwarded-Proto: https must pass through with no redirect."""
        with patch("config.IS_PRODUCTION", True):
            with app.test_client() as c:
                resp = c.get(
                    "/",
    ...
    def test_http_request_not_redirected_in_dev(self):
        """Outside production the redirect hook must not fire, even with the header."""
        with patch("config.IS_PRODUCTION", False):
            with app.test_client() as c:
                resp = c.get(
                    "/",
                    headers={"X-Forwarded-Proto": "http"},
