...
# ---------------------------------------------------------------------------
# Batch F #6 — recording links opened where nobody is signed in (e.g. the email
# on a phone) bounce to /login and come back, instead of a dead "Forbidden".
# ---------------------------------------------------------------------------
...
def test_download_redirects_to_login_when_not_signed_in(enc_client):
    ...
    with enc_client.session_transaction() as s:
        assert s.get("post_login_redirect", "").startswith("/recording/download/dltok")


def test_download_doc_redirects_to_login_when_not_signed_in(enc_client):
    with app.app_context():
        sid = _seed(); _seed_recording(sid, token="dltok")
    with patch.object(config, "RECORDING_ENABLED", True):
        rv = enc_client.get("/recording/download/dltok/pdf")
    assert rv.status_code == 302
    assert "/login" in rv.headers["Location"]
...
# ---------------------------------------------------------------------------
# Batch F #7 — the therapist ending the session also ends the recording for
# everyone (stops egress and finalizes), not just the chat.
# ---------------------------------------------------------------------------

def test_end_session_stops_active_recording(enc_client):
    with app.app_context():
        sid = _seed("ther-1")
