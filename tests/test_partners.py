...
from models import db, init_encryption, Clinician, Partner, AuditLog
...
def _stripe_ok(promo_id="promo_1"):
    ...
    return patch.object(billing, "create_partner_code",
                        return_value=MagicMock(id=promo_id))


def _add(**kw):
    args = dict(name="Easton", email="easton@example.com",
                discount_pct=10, commission_pct=40, max_uses=None,
                admin_email=ADMIN)
    args.update(kw)
    return admin_access.create_partner(db, Partner, **args)
...
def test_a_normal_split_is_accepted():
    assert admin_access.partner_split_error(10, 40) is None


def test_the_two_shares_need_not_add_to_fifty():
    """The admin sets both numbers; nothing is computed from the other."""
    assert admin_access.partner_split_error(5, 10) is None
    assert admin_access.partner_split_error(30, 30) is None
...
def test_a_split_that_leaves_nothing_is_refused():
    ...
    assert admin_access.partner_split_error(50, 50) is not None
...
def test_a_split_that_leaves_less_than_the_card_fee_is_refused():
    ...
    assert admin_access.partner_split_error(60, 38) is not None    # keeps 2%
    assert admin_access.partner_split_error(60, 35) is None        # keeps 5%


def test_a_discount_outside_one_to_a_hundred_is_refused():
    assert admin_access.partner_split_error(0, 10) is not None
    assert admin_access.partner_split_error(101, 0) is not None


def test_junk_percentages_are_refused_rather_than_crashing():
    assert admin_access.partner_split_error("ten", 40) is not None
    assert admin_access.partner_split_error(None, None) is not None
...
def test_the_code_carries_a_random_ending():
    ...
    the bigger discount."""
    a = billing.suggest_partner_code("Easton")
    b = billing.suggest_partner_code("Easton")
...
def test_the_code_only_uses_characters_stripe_allows():
    ...
    for name in ("Easton O'Neill", "Zoë & Co.", "  spaces  ", ""):
        assert re.fullmatch(r"[A-Za-z0-9-]+", billing.suggest_partner_code(name))
...
def test_the_code_avoids_letters_that_are_misread_aloud():
    ...
    wrong."""
    for _ in range(20):
        tail = billing.suggest_partner_code("X").split("-")[1]
...
def test_partners_sharing_a_discount_share_one_coupon():
    ...
    assert billing.partner_coupon_id(10) == billing.partner_coupon_id(10)
    assert billing.partner_coupon_id(10) != billing.partner_coupon_id(20)
...
def test_the_max_uses_cap_is_handed_to_stripe(client):
    ...
    with app.app_context():
        with patch.object(billing, "create_partner_code",
...
def test_a_bad_split_never_reaches_stripe(client):
    with app.app_context():
        with patch.object(billing, "create_partner_code") as mk:
...
def test_stopping_a_partner_switches_the_code_off_in_stripe(client):
    with app.app_context(), _stripe_ok("promo_live"):
        ...
        with patch.object(billing, "deactivate_promotion_code") as off:
            assert admin_access.stop_partner(db, Partner, row.id) is True
        off.assert_called_once_with("promo_live")
        assert db.session.get(Partner, row.id).active is False
...
def test_stopping_twice_is_a_no_op(client):
    with app.app_context(), _stripe_ok():
        ...
        with patch.object(billing, "deactivate_promotion_code"):
            admin_access.stop_partner(db, Partner, row.id)
            assert admin_access.stop_partner(db, Partner, row.id) is False
...
def test_adding_through_the_console_is_written_to_the_audit_log(client):
    def check():
        with _stripe_ok("promo_c"):
            client.post("/accessadmin/partner",
                        data={"name": "Easton", "email": "e@example.com",
                              "discount_pct": "10", "commission_pct": "40",
                              "max_uses": "5"},
                        follow_redirects=True)
        with app.app_context():
            assert Partner.query.count() == 1
            assert AuditLog.query.filter_by(event_type="partner_added").count() == 1
    _as_verified_admin(client, check)


def test_a_bad_split_is_reported_and_nothing_is_saved(client):
    def check():
        html = client.post("/accessadmin/partner",
                           data={"name": "Easton", "discount_pct": "50",
                                 "commission_pct": "50"},
...
def test_the_card_is_hidden_when_billing_is_off(client):
    def check():
        with patch.object(config, "BILLING_ENABLED", False):
            html = client.get("/accessadmin").get_data(as_text=True)
        assert "Partners" not in html
    _as_verified_admin(client, check)


def test_an_unverified_admin_cannot_add_a_partner(client):
    patches = [patch.object(config, "ADMIN_EMAILS", [ADMIN]),
               patch.object(config, "ADMIN_CONSOLE_ENABLED", True),
               patch.object(config, "BILLING_ENABLED", True)]
    for p in patches:
    ...
    try:
        ...
        rv = client.post("/accessadmin/partner",
                         data={"name": "Sneaky", "discount_pct": "10",
                               "commission_pct": "40"})
